	...
	stages{
		...
		agent { label 'dhfLinuxAgent'}
			steps{
				...
				println(BRANCH_NAME)
				sh 'echo '+JAVA_HOME+';export PATH=$WORKSPACE'+GRADLE_USER_HOME+':'+JAVA_HOME+'/bin:$PATH:$MAVEN_HOME/bin;cd $WORKSPACE/data-hub;rm -rf $WORKSPACE'+GRADLE_USER_HOME+'/caches;./gradlew clean --stacktrace;./gradlew build -x test -Pskipui=true;'
